...
- Name: Authentication System >> Language Selector Functionality
- Location: /var/www2/tests/auth.spec.js:202:3
...
Locator: locator('button img[src*="flag-"]')
Expected: 3
Received: 0
Call log:
  - expect.toHaveCount with timeout 5000ms
  - waiting for locator('button img[src*="flag-"]')
    9 × locator resolved to 0 elements
      - unexpected value "0"

    at /var/www2/tests/auth.spec.js:210:25
```
...
```ts
  ...
  125 |     expect(profileData.user.email).toBe(TEST_USER.email);
  126 |     expect(profileData.user.firstName).toBe(TEST_USER.firstName);
  127 |   });
  128 |
  129 |   test('Token Refresh Flow', async ({ request }) => {
  130 |     // Login to get tokens
  131 |     const loginResponse = await request.post(`${BASE_URL}/api/auth/login`, {
  132 |       data: {
...
> 210 |     await expect(flags).toHaveCount(3);
  ...
  242 |     const hamburger = page.locator('button').filter({ hasText: /menu|☰/ }).or(
